...
const StockpileStatsSidebar = ({ stats }: { stats: any }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="border-none shadow-sm bg-white dark:bg-slate-900 rounded-[32px] p-6 transition-colors">
        <div className="flex items-center gap-3 mb-6">
          <Truck className="w-5 h-5 text-gray-400 dark:text-slate-500" />
          <h3 className="font-bold text-lg dark:text-white">Delivery status</h3>
        </div>
            ...
            <PieChart>
              ...
              >
                {deliveryData.map((entry, index) => (
                  <Cell key={`cell-${index}`} fill={index === 0 ? '#F97316' : (document.documentElement.classList.contains('dark') ? '#1E293B' : '#E5E7EB')} />
                ))}
              </Pie>
            </PieChart>
          </ResponsiveContainer>
          <div className="absolute bottom-0 left-1/2 -translate-x-1/2 text-center pb-4">
            <p className="text-[10px] font-bold text-muted-foreground dark:text-slate-500 uppercase">Total Deliveries</p>
            <p className="text-xl font-black dark:text-white">{stats.totalStockpileCount}</p>
          </div>
        </div>

        <div className="grid grid-cols-2 gap-4 mt-6 pt-6 border-t border-orange-50 dark:border-slate-800">
          <div className="text-center">
            ...
            <p className="text-[10px] font-bold text-muted-foreground dark:text-slate-500 uppercase">Unpaid</p>
            <p className="font-black text-sm dark:text-white">{stats.deliveryUnpaidCount}</p>
          </div>
          <div className="text-center">
            <div className="w-8 h-8 rounded-lg bg-gray-50 dark:bg-slate-800 flex items-center justify-center mx-auto mb-2">
              <CheckCircle2 className="w-4 h-4 text-gray-400 dark:text-slate-500" />
            </div>
            <p className="text-[10px] font-bold text-muted-foreground dark:text-slate-500 uppercase">Paid</p>
            <p className="font-black text-sm dark:text-white">{stats.deliveryPaidCount}</p>
          </div>
        </div>
      </Card>
      ...
      <Card className="border-none shadow-sm bg-white dark:bg-slate-900 rounded-[32px] p-6 transition-colors">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h3 className="font-bold text-lg dark:text-white">Stockpile quantity</h3>
        </div>
        ...
        <div className="relative h-[200px] w-full">
          ...
          <div className="absolute bottom-0 left-1/2 -translate-x-1/2 text-center pb-4">
            <p className="text-[10px] font-bold text-muted-foreground dark:text-slate-500 uppercase">All stockpile</p>
            <p className="text-xl font-black dark:text-white">{stats.totalStockpileCount}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 mt-6 pt-6 border-t border-orange-50 dark:border-slate-800">
          <div className="text-center">
            ...
            <p className="text-[10px] font-bold text-muted-foreground dark:text-slate-500 uppercase">Open stockpile</p>
            <p className="font-black text-sm dark:text-white">{stats.openStockpileCount}</p>
          </div>
          <div className="text-center">
            <div className="w-8 h-8 rounded-lg bg-gray-50 dark:bg-slate-800 flex items-center justify-center mx-auto mb-2">
              <DollarSign className="w-4 h-4 text-gray-400 dark:text-slate-500" />
            </div>
            <p className="text-[10px] font-bold text-muted-foreground dark:text-slate-500 uppercase">Closed stockpile</p>
            <p className="font-black text-sm dark:text-white">{stats.closedStockpileCount}</p>
          </div>
        </div>
      </Card>
    </div>
    ...
    <div className="min-h-screen dashboard-bg font-sans transition-colors">
      ...
      <main className="max-w-[1600px] mx-auto p-4 md:p-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-2">
          <div>
            <h1 className="text-3xl md:text-4xl font-bold font-heading mb-2 dark:text-white">Stockpile</h1>
            <p className="text-muted-foreground dark:text-slate-400">you can view and manage your stockpile here</p>
          </div>
          <div className="flex items-center gap-4">
            <AnimatePresence>
              {selectedIds.length > 0 && (
                <motion.div 
                  ...
                  className="flex items-center gap-2 bg-white dark:bg-slate-900 border border-orange-100 dark:border-slate-800 p-1.5 rounded-full shadow-lg transition-colors"
                >
                  <div className="px-4 py-1.5 bg-orange-50 dark:bg-orange-950/30 rounded-full">
                    <span className="text-sm font-black text-cartlist-orange">{selectedIds.length} selected</span>
                  </div>
                  <div className="h-6 w-px bg-orange-100 dark:bg-slate-800 mx-1" />
                  <Button 
                    variant="ghost" 
                    size="sm" 
            ...
            <Button 
              ...
              className="bg-cartlist-orange hover:bg-orange-600 text-white rounded-full px-8 h-12 font-bold gap-2 shadow-lg shadow-orange-200 dark:shadow-none"
            >
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="flex flex-col md:flex-row gap-4">
              <div className="relative flex-1">
                ...
                <Input 
                  placeholder="Search ..." 
                  className="pl-12 h-12 bg-white dark:bg-slate-900 border-orange-50 dark:border-slate-800 rounded-2xl focus-visible:ring-cartlist-orange shadow-sm dark:text-white transition-colors"
                  value={search}
              ...
              <div className="flex gap-4">
                <Select value={sortFilter} onValueChange={setSortFilter}>
                  <SelectTrigger className="w-[160px] h-12 bg-white dark:bg-slate-900 border-orange-50 dark:border-slate-800 rounded-2xl font-bold shadow-sm dark:text-white transition-colors">
                    <div className="flex items-center gap-2">
                      <Filter className="w-4 h-4" />
                      <SelectValue placeholder="Filter" />
                    </div>
                  </SelectTrigger>
                  <SelectContent className="rounded-2xl p-2 border-orange-100 dark:border-slate-800 dark:bg-slate-900">
                    <SelectItem value="today" className="rounded-xl dark:text-white focus:bg-slate-800">Today</SelectItem>
                    <SelectItem value="oldest" className="rounded-xl dark:text-white focus:bg-slate-800">Oldest to Newest</SelectItem>
                    <SelectItem value="newest" className="rounded-xl dark:text-white focus:bg-slate-800">Newest to Oldest</SelectItem>
                    <SelectItem value="thisMonth" className="rounded-xl dark:text-white focus:bg-slate-800">This month</SelectItem>
                    <SelectItem value="closingSoon" className="rounded-xl dark:text-white focus:bg-slate-800">Closing soon</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            </div>
                        ...
                        <div 
                          ...
                          className={`w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all cursor-pointer ${
                            selectedIds.length > 0 && selectedIds.length === stockpiles.length
                              ...
                              : selectedIds.length > 0
                              ? "bg-orange-200 dark:bg-orange-950 border-cartlist-orange"
                              : "border-gray-300 dark:border-slate-700 hover:border-cartlist-orange"
                          }`}
                        >
                          {selectedIds.length > 0 && (
                      ...
                      Array.from({ length: 5 }).map((_, i) => (
                        <tr key={i} className="animate-pulse">
                          <td colSpan={9} className="px-6 py-8">
                            <div className="h-10 bg-gray-100 dark:bg-slate-800 rounded-xl w-full" />
                          </td>
                        </tr>
                      ))
                    ) : stockpiles.length > 0 ? (
                        ...
                        <tr 
                          ...
                          className={`transition-all duration-200 group cursor-pointer border-l-4 ${
                            selectedIds.includes(stockpile._id) 
                              ? "bg-orange-50/60 dark:bg-orange-950/10 border-cartlist-orange" 
                              : "hover:bg-gray-50/80 dark:hover:bg-slate-800/50 border-transparent"
                          }`}
                          onClick={() => {
                        ...
                        >
                            ...
                            <div 
                              ...
                              className={`w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all duration-200 cursor-pointer ${
                                selectedIds.includes(stockpile._id)
                                  ...
                                  : "border-gray-300 dark:border-slate-700 group-hover:border-cartlist-orange/50"
                              }`}
                            >
                              {selectedIds.includes(stockpile._id) && (
                                <CheckCircle2 className="w-3.5 h-3.5 text-white" />
                          ...
                          <td className="px-4 py-5">
                            <div className="flex items-center gap-3">
                              <div className="w-10 h-10 rounded-full bg-orange-100 dark:bg-orange-950 flex items-center justify-center overflow-hidden border-2 border-white dark:border-slate-800 shadow-sm">
                                <img src={`https://api.dicebear.com/7.x/lorelei/svg?seed=${stockpile.customerName}`} alt="Avatar" />
                              </div>
                              <div>
                                <p className="text-sm font-bold text-gray-900 dark:text-white group-hover:text-cartlist-orange transition-colors">{stockpile.customerName}</p>
                                <p className="text-[10px] text-muted-foreground dark:text-slate-500 font-medium">{stockpile.customerPhone}</p>
                              </div>
                            </div>
                          </td>
                          <td className="px-4 py-5">
                            <p className="text-sm font-black text-gray-900 dark:text-white">₦{stockpile.totalAmount.toLocaleString()}</p>
                            <p className="text-[10px] text-muted-foreground dark:text-slate-500 font-bold uppercase tracking-wider">{stockpile.items.length} items</p>
                          </td>
                          <td className="px-4 py-5">
                            <p className="text-sm font-bold text-gray-700 dark:text-slate-300">{new Date(stockpile.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
                            <p className="text-[10px] text-muted-foreground dark:text-slate-500 font-medium">
                          ...
                          <td className="hidden md:table-cell px-4 py-5 w-40">
                            {(() => {
                              ...
                              return (
                                <div className="flex items-center gap-3">
                                  <div className="flex-1 h-2 bg-gray-100 dark:bg-slate-800 rounded-full overflow-hidden border border-gray-50 dark:border-slate-700">
                                    <div 
                                  ...
                                  <span className={`text-[10px] font-black whitespace-nowrap ${
                                    daysLeft <= 2 ? "text-red-500" : "text-muted-foreground dark:text-slate-500"
                                  }`}>{daysLeft}d</span>
                                </div>
                              );
                            })()}
                          ...
                          <td className="hidden md:table-cell px-4 py-5">
                            <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl w-fit font-bold border ${
                              stockpile.status === "active" 
                                ? "bg-orange-50 dark:bg-orange-950/30 text-cartlist-orange border-orange-100 dark:border-orange-800 shadow-sm shadow-orange-50" 
                                : "bg-green-50 dark:bg-green-950/20 text-green-600 border-green-100 dark:border-green-800 shadow-sm shadow-green-50"
                            }`}>
                          ...
                          <td className="hidden md:table-cell px-4 py-5">
                            <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl w-fit font-bold border ${
                              stockpile.deliveryPaid 
                                ? "bg-blue-50 dark:bg-blue-950/20 text-blue-600 border-blue-100 dark:border-blue-800 shadow-sm shadow-blue-50" 
                                : "bg-red-50 dark:bg-red-950/20 text-red-600 border-red-100 dark:border-red-800 shadow-sm shadow-red-50"
                            }`}>
                          ...
                          <td className="px-6 md:px-8 py-5 text-right" onClick={(e) => e.stopPropagation()}>
                            <DropdownMenu>
                              <DropdownMenuTrigger asChild>
                                <Button variant="ghost" size="icon" className="rounded-full hover:bg-orange-50 dark:hover:bg-slate-800 transition-colors">
                              ...
                              <DropdownMenuContent align="end" className="rounded-2xl p-2 border-orange-100 dark:border-slate-800 shadow-xl bg-white dark:bg-slate-900 z-[200]">
                                <DropdownMenuItem 
                                  ...
                                  className="rounded-xl px-3 py-2 cursor-pointer flex items-center gap-3 hover:bg-orange-50 dark:hover:bg-slate-800 group/item dark:text-white"
                                >
